...
<head>
    ...
    <title>Reset Password - Our Platform</title>
    ...
    <main class="flex-grow flex flex-col justify-center items-center pt-24 pb-16 px-4">
        ...
        <div class="w-full max-w-md">
            <div class="backdrop-blur-lg bg-[#0b1f35]/70 border border-white/20 shadow-2xl rounded-2xl p-8 md:p-10">
                <h2 class="text-3xl font-bold text-white text-center mb-8">Reset Your Password</h2>
                ...
                <form method="POST" action="{{ route('password.store') }}">
                    ...
                    <div>
                        ...
                        <input id="email" class="block mt-1 w-full rounded-xl p-3 bg-black/20 text-white placeholder-gray-400 border-white/20 focus:ring-purple-500 focus:border-purple-500" type="email" name="email" :value="old('email', $request->email)" required autofocus autocomplete="username" />
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <label for="password" class="block text-sm font-medium text-gray-300 mb-1">{{ __('Password') }}</label>
                        <input id="password" class="block mt-1 w-full rounded-xl p-3 bg-black/20 text-white placeholder-gray-400 border-white/20 focus:ring-purple-500 focus:border-purple-500" type="password" name="password" required autocomplete="new-password" />
